...
window.ArcadeNet = (function () {
  ...
  if (socket) {
    ['presence', 'score:update', 'cabinet:update',
      ...
      'rt:opponent_left', 'rt:error',
      'pb:waiting', 'pb:joined', 'pb:state', 'pb:shot', 'pb:tagged', 'pb:damaged', 'pb:respawn', 'pb:left', 'pb:error'].forEach((evt) => {
  ...
  const pb = {
    available: !!socket,
    quickMatch(name, avatar) { if (socket) socket.emit('pb:quickmatch', { name, avatar }); },
    createRoom(name, avatar) { if (socket) socket.emit('pb:create', { name, avatar }); },
    joinRoom(code, name, avatar) { if (socket) socket.emit('pb:join', { code, name, avatar }); },
    move(x, y) { if (socket) socket.emit('pb:move', { x, y }); },
    shoot(targetX, targetY) { if (socket) socket.emit('pb:shoot', { targetX, targetY }); },
    leave() { if (socket) socket.emit('pb:leave'); },
  };

  return {
    hasServer, on, fetchFloor, postScore, postCabinet, deleteCabinet, mp, rt, pb,
  };
}());
